...
public class ProductDAOImpl implements ProductDAO {
	...
	@Override
	public List<ProductDTO> viewProductByCategories(Connection conn, List<Integer> categoryIds, String sort) throws SQLException {
		...
		switch (sort) {
		case "latest":
			orderBy = "p.CREATED DESC, p.PRODUCT_ID";
			break;

		case "lowprice":
			orderBy = "p.PRICE ASC, p.PRODUCT_ID";
			break;

		case "recommend":
			orderBy = "avg_rating DESC, p.PRODUCT_ID";
			break;

		case "popular":	     
			orderBy = "NVL(s.sales_quantity, 0) DESC, p.product_id";
			break;
		...
		String sql = """
				SELECT p.product_id,
					...
					NVL(s.sales_quantity, 0) AS sales_quantity
				FROM product p
				JOIN brand b
				ON p.brand_id = b.brand_id
					LEFT JOIN (
